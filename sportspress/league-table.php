...
if(isset($ws['table_style'])){ $table_style = $ws['table_style']; } else {
...
if ( $title )
	$output .= '<h4 class="sp-table-caption skpbg skwc">' . $title . '</h4>';

$output .= '<div class="sp-table-wrapper  table-responsive">';

$output .= '<table class="sp-league-table table sda '.$table_style.' sk-league-table sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table '.$identifier : '' ). ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr class="font-secondary">';
...
//$output .= '<th class="data-rank">' . __( 'Pos', 'skyre' ) . '</th>';
if(!empty($ws)) { 
	foreach( $columns as $key => $label ):
		$output .= '<th class="data-' . $key . '">' . $labels[$label] . '</th>';
	endforeach;
}else {
	$columns['Pos'] = 'pos';
	$columns['club'] = 'name';
...
foreach ( $data as $team_id => $row ):
	...
	if(!empty($ws)) { 
		foreach( $columns as $key => $value ):
			if($value == 'name'){ $output .= '<td class="data-name' . $name_class . $td_class . '" data-label="'.$labels['name'].'">' . $name . '</td>'; continue; }
			$output .= '<td class="data-' . $value . $td_class . '" data-label="'.$labels[$value].'">' . sp_array_value( $row, $value, '&mdash;' ) . '</td>';
		endforeach;
	}else {
		foreach( $labels as $key => $value ):
			if ( ! is_array( $columns ) || in_array( $key, $columns ) ){
				if($key == 'name'){ $output .= '<td class="data-name' . $name_class . $td_class . '" data-label="'.$labels['name'].'">' . $name . '</td>'; continue; }
...
if ( $show_full_table_link && $id !=1 )
	$output .= '<div class="sp-league-table-link sk-sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . __( 'View full table', 'skyre' ) . '</a></div>';
?>
